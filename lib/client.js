...
const MAGIC = constant.magic;
const OPCODE = constant.opcode;
const ROPCODE = constant.ropcode;
...
class Client extends EventEmitter {
  constructor(options) {
    ...
    Command.decodeFromStream(this.connection, (err, cmd) => {
      ...
      if (cmd.status) {
        let err = new Error(STATUS[cmd.status]);
        err.code = cmd.status;
  ...
  close() {
    this.send(OPCODE.QUIT, function(err, cmd) {
      if (err) {
        this.emit('error', err);
      }
    });
  ...
  doAuth() {
    this.send(OPCODE.SASL_LIST_MECHS, (err, v) => {
      if (err) {
  ...
  reconnect() {
    ...
    if (++this.times > 3) {
      ...
      this.emit('close', this.lastError);
