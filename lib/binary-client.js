...
class Client extends Base {
  constructor(options) {
    ...
    Command.decodeFromStream(this.connection, (err, cmd) => {
      ...
      const opaque = cmd.opaque;
      if (cmd.status) {
        const err = new Error(`The server returns an error: ${STATUS[cmd.status]}`);
        err.code = cmd.status;
        this.callCallback(opaque, err, null);
      } else {
        const opcode = cmd.opcode;
        if (opcode === OPCODE.GET || opcode === OPCODE.GAT) {
          const flags = cmd.extras.readUInt32BE(0);
          ...
          const buf = cmd.value;
          const long = new Long(buf.readUInt32BE(4), buf.readUInt32BE(0), true);
  ...
  cleanupCallbacks(err) {
    for (const opaque in this.callbacks) {
      this.callCallback(opaque, err);
    }
  }

  createConnection() {
    const connection = this.connection = new net.Socket();
  ...
  reconnect() {
    ...
    if (++this.times > 9999) {
      ...
      const timeout = this.times * 2 * 1000;
